...
library Rlp {
	...
	/// @return An Item
	function toItem(bytes memory self) internal view returns (Item memory) {
	...
	/// @return An Item
	function toItem(bytes memory self, bool strict) internal view returns (Item memory) {
	...
	function rlpBytesToUint256(bytes memory source) internal view returns (uint256 result) {
		Item memory item = Rlp.toItem(source);
		console.log('rlpBytesToUint256');
		return Rlp.toUint(item);
	}
}
